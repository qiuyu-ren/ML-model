...
def train_model():
    ...
    num_epochs, patience, burn_in_steps = 50, 300, 1
...
def run_ablation_experiment():
    ...
    # 准备测试集
    test_datasets = [ThermalDataset(f) for f in test_paths]

    results = []
    data_ratios = [i / 10 for i in range(10, 0, -1)]  # [1.0, 0.9, ..., 0.1]

    for ratio in data_ratios:
        print(f"\n=== Training with {int(ratio*100)}% data ===")

        num_train = int(len(train_paths) * ratio)
        selected_train_paths = train_paths[:num_train]

        train_dfs = [pd.read_csv(f) for f in selected_train_paths]
        scaler = MinMaxScaler()
        scaler.fit(pd.concat(train_dfs)[["Time (s)", "T_outer (C)", "T_inner (C)", "T_ave (C)"]])
        ...
        # 简化验证集为最后5%
        val_split = int(0.05 * len(train_paths))
        val_paths = train_paths[-val_split:] if val_split > 0 else train_paths[-1:]
        val_datasets = [ThermalDataset(f, scaler=scaler) for f in val_paths]
        val_loader = DataLoader(ConcatDataset(val_datasets), batch_size=16)
        ...
        results.append((int(ratio * 100), ave_train_loss, ave_val_loss))

    # 绘图
    percentages, train_losses, val_losses = zip(*results)
    plt.plot(percentages, train_losses, marker='o', label="Train Loss")
    plt.plot(percentages, val_losses, marker='o', label="Val Loss")
    plt.xlabel("Training Data Percentage")
    plt.ylabel("Loss")
    plt.title("Effect of Data Reduction on Loss")
    plt.legend()
    plt.gca().invert_xaxis()  # 从100%到10%从左往右
    plt.grid(True)
    plt.tight_layout()
    plt.show()
