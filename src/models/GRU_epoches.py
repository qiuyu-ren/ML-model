...
# === 训练函数（接受 max_epochs） ===
def train_model(max_epochs=250):
...
def test_model(model, test_datasets, scaler, epoch_tag=""):
    ...
    for dataset in test_datasets:
        ...
        plt.title(f"[{epoch_tag}] Prediction - {file}")
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"result_{epoch_tag}_{file}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    for epoch_num in epoch_list:
        ...
        model, test_sets, scaler = train_model(max_epochs=epoch_num)
